  ...
  return (
    <section id="philosophy" className="relative z-20 scroll-mt-24 px-4 pb-6 pt-16 sm:px-6">
      <div className="mx-auto max-w-3xl text-center">
        ...
        <h2 className="font-display mt-6 text-4xl font-medium italic text-cream sm:text-6xl">
      ...
      <article className="relative mx-auto mt-10 max-w-3xl overflow-hidden rounded-[28px] bg-[#FCF8F2] text-[#141A17] shadow-[0_30px_60px_-24px_rgba(0,0,0,0.55)]">
        <div className="h-3 bg-[#8E1C24]" />
              ...
              <li key={item.course} className="grid gap-1 sm:grid-cols-[4.5rem_1fr] sm:gap-4">
                ...
                <div>
                  <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
                    <h3 className="font-display text-2xl italic leading-tight">{item.role}</h3>
...
function TeaCup() {
  return (
    <svg viewBox="0 0 64 64" className="h-12 w-12" aria-hidden="true">
      <ellipse cx="32" cy="54" rx="16" ry="4" fill="#141A17" opacity="0.25" />
      <ellipse cx="32" cy="48" rx="18" ry="6" fill="#CADB66" />
      <path d="M16 28c0-2 4-4 16-4s16 2 16 4v16c0 6-6 10-16 10S16 50 16 44Z" fill="#FCF8F2" />
      <ellipse cx="32" cy="28" rx="16" ry="5" fill="#3a6b32" />
      <ellipse cx="32" cy="27" rx="10" ry="3" fill="#CADB66" opacity="0.35" />
      <path d="M48 32c8 2 10 10 4 14" fill="none" stroke="#FCF8F2" strokeWidth="3" strokeLinecap="round" />
      <path d="M26 18c2-6 4-8 4-10" fill="none" stroke="#FCF8F2" strokeWidth="2" opacity="0.5" />
      <path d="M34 16c1-5 3-8 2-11" fill="none" stroke="#FCF8F2" strokeWidth="2" opacity="0.35" />
    </svg>
  )
}
...
export function OrderFooter() {
  ...
  return (
    <section id="contact" className="relative z-20 scroll-mt-24 px-4 pb-16 pt-10 text-center sm:px-6">
      <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-[#8E1C24] ring-2 ring-lime/70">
        <TeaCup />
      </div>
      <h2 className="font-display text-4xl italic text-cream sm:text-5xl">Ready to place an order?</h2>
      ...
      <div className="mx-auto mt-8 flex max-w-xl flex-wrap items-center justify-center gap-3">
        {links.map((link) => (
          <a
            ...
            className={
              link.primary
                ? 'rounded-full bg-cream px-6 py-3 font-type text-sm text-mahogany hover:brightness-110'
                : 'rounded-full border border-cream/20 bg-[#8E1C24] px-6 py-3 font-serif text-lg italic text-cream hover:border-lime hover:text-lime'
            }
          >
            {link.label}
          </a>
